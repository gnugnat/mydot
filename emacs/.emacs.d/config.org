...
#+BEGIN_SRC emacs-lisp
  ...
  (global-set-key (kbd "C-= C-c c") 'config-visit)

  (defun gui-visit ()
    "Edit gui-config.org."
    (interactive)
    (find-file "~/.emacs.d/gui-config.org")
    )
...
#+BEGIN_SRC emacs-lisp
  ...
  (defun gui-reload ()
    ...
    (interactive)
    (progn
      (if (file-exists-p "~/.emacs.d/gui-config.el")
          (delete-file "~/.emacs.d/gui-config.el")
        )
      (gui-load)
      )
    )
...
** Python
   ...
   DEBUG: If =~/.emacs.d/elpy/rpc_venv= is empty remove it and run =elpy-config=.
   + Elpy:
...
#+BEGIN_SRC emacs-lisp
  (use-package elpy
    ...
    (progn
      (setq
       elpy-rpc-virtualenv-path "~/.emacs.d/elpy/rpc_venv"
